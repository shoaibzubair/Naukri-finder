...
function naukriSkillsFilter() {
    ...
    const startFullSearch = () => {
      ...
      maxPages = 50 //getMaxPages();
      
      // Set max pages to search
      const maxPagesToSearch = Math.min(config.maxPagesToSearch, maxPages);
      
      // Update full search button
      ...
      // Start searching
      isSearching = true;
      console.log(`Starting full search through ${maxPagesToSearch} pages...`);
      
      // Start the search process
      searchCurrentPage(currentPage, maxPagesToSearch);
    };
    
    // Search the current page and then move to the next
    const searchCurrentPage = (currentPageNum, maxPagesToSearch) => {
      console.log(`Searching page ${currentPageNum}...`);
      
      // Update progress display
      updateProgress(currentPageNum, maxPagesToSearch);
      ...
      if (currentPageNum < maxPagesToSearch) {
        ...
        if (hasNextPage) {
          ...
          setTimeout(() => {
            searchCurrentPage(currentPageNum + 1, maxPagesToSearch);
          }, config.searchDelay);
        } else {
